...
{       
		VVLog( "* Change Found! Reloading File \"%s\"", srcFile.filename.c_str() );
        size_t counter = 0;
        ...
        while (!feof( fp ))
        {
                ...
                while ((ch=strstr( ch, "_T" "V(")) )
                {
                        ...
                        if (chend)
                        {
                                ...
                                VLog("TWEAK: %s, %d : val '%s'", srcFile.filename.c_str(), counter, strval );
                                ch = chend;
...
void ReloadChangedTweakableValues()
{
                  ...
                  ti != g_tweakableFile.end(); ++ti )
        {
                ...
                VVVLog("* Scaning \"%s\"", src.filename.c_str() );
