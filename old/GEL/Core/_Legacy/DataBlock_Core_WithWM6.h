...
#ifdef USES_WINDOWSMOBILE
	//extern void* vMem;
	//DataBlock* p = reinterpret_cast<DataBlock*>(VirtualAlloc( vMem, _Size + sizeof(DataBlock), MEM_COMMIT, PAGE_READWRITE ));
	void* vMem = VirtualAlloc( NULL, _Size + sizeof(DataBlock), MEM_RESERVE, PAGE_NOACCESS );
	DataBlock* p = reinterpret_cast<DataBlock*>(VirtualAlloc( vMem, _Size + sizeof(DataBlock), MEM_COMMIT, PAGE_READWRITE ));
...
#ifdef USES_WINDOWSMOBILE
	//VirtualFree( p, p->Size + sizeof(DataBlock), MEM_DECOMMIT );
	VirtualFree( p, p->Size + sizeof(DataBlock), MEM_DECOMMIT );
