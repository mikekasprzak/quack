...
#	define RDE_ASSERT	assert
	...
	{ 
		...
		namespace Sys 
		{
			RDE_FORCEINLINE void MemCpy(void* to, const void* from, std::size_t bytes)
			{
				std::memcpy(to, from, bytes);
			}
			RDE_FORCEINLINE void MemMove(void* to, const void* from, std::size_t bytes)
			{
				std::memmove(to, from, bytes);
			}
			RDE_FORCEINLINE void MemSet(void* buf, unsigned char value, std::size_t bytes)
			{
